...
/** Client for the TransferSimulator available in the college network. */
public class TransferSimulatorClient {
    public static final String BASE_URL = "http://192.168.1.200:4444/TransferSimulator/";
    ...
    public CompletableFuture<ApiResponse> request(ApiDataType dataType) {
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(BASE_URL + dataType.endpoint()))
                .timeout(Duration.ofSeconds(10))
                .GET()
                .build();

        return httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                .thenApply(response -> parseResponse(dataType, response));
    }

    private ApiResponse parseResponse(ApiDataType dataType, HttpResponse<String> response) {
        ...
        if (!matcher.find()) {
            throw new IllegalStateException("Сервер вернул ответ без поля value.");
        }
